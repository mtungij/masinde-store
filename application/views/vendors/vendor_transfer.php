...
<head>
  ...
  <!-- Title  -->
  <title>Masinde Store| Transfer to vendor</title>
    ...
    <div class="main-content flex-grow min-h-[100%] py-20 relative px-4 lg:pr-8 lg:pl-3">
      ...
      <div class="bg-white max-w-full rounded-lg p-2 lg:p-4 ">
        ...
        <?php endif;?>
         ...
         <?php echo form_open('vendors/transfer_to_vendor', ["class" => "my-2 grid grid-cols-1 items-start gap-4 md:grid-cols-2 lg:grid-cols-3"])?>
         <input type="hidden" name="branch_id" value="<?= $branchId ?>">
         <div class="relative z-0">
            <select class="select2 w-full" name="product_id" style="padding: 4px">
              <option value="">--Select Product--</option>
              <?php foreach($products as $product):?>
                <option value="<?= $product->id?>"><?= $product->product_name ?> ~ <span style="color: green"><?= $product->inventory ?></span></option>
              <?php endforeach;?>
            </select>
          </div>
          ...
          <div class="relative z-0">
            <select id="transfer_to" name="vendor_id" class="w-full h-12 block leading-5 relative py-2 px-4 rounded bg-neutral-10 dark:bg-neutral-900 border focus:border-2 border-gray-500 overflow-x-auto focus:outline-none focus:border-primary-600 focus:ring-0 dark:text-gray-200 dark:border-gray-400 dark:focus:border-primary-200">
        ...
        <div class="overflow-x-auto" style="white-space:nowrap;">
         <?php if($this->session->flashdata('vp_confirmation')):?>
          <div class="flex items-center gap-2 relative bg-green-50 text-green-700 p-4 my-2 border rounded">
            <i class="material-symbols-outlined">info</i>
            <p><?= $this->session->flashdata('vp_confirmation') ?></p>
          </div>
          <?php endif ?>
            ...
            <tbody>
              ...
              <tr class="[&.selected]:!bg-primary-100 dark:[&.selected]:!bg-primary-700">
                ...
                <td class="font-normal text-center">
                    <a href="<?= site_url('vendors/delete/'.$transfer->id) ?>">
