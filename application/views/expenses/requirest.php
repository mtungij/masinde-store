  ...
  <main class="lg:flex">
    ...
    <div class="main-content flex-grow min-h-[100%] py-20 relative px-4 lg:pr-8 lg:pl-3">
      ...
      <div class="flex flex-row justify-between items-center pt-2 pb-6">
        <h2 class="text-title-lg">Request Expenses</h2>
      </div>
        ...
        <div class="py-8 px-6 flex flex-col rounded-xl bg-white dark:bg-gray-900">
          ...
          <div class="tabs flex flex-col w-full">
            ...
            <div class="relative flex flex-row items-center">
              ...
              <?php elseif($this->session->flashdata('expenses_success')): ?>
                  <div class="flex items-center gap-2 relative bg-green-50 text-green-700 p-4 rounded">
                    <i class="material-symbols-outlined">info</i>
                    <p><?= $this->session->flashdata('expenses_error');
                     ?></p>
                  </div>
              <?php endif?>
            </div>
            ...
            <div class="flex flex-col">
              <?php echo form_open('Expenses/request_submit', ['id' => 'tab-4', 'role' => 'tabpanel', 'class' => 'active [&.active]:block hidden py-4 transition duration-400 ease-in-out'])?>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-8">
                 
                <div class="relative z-0">
                        <select id="branch_id" name="branch" value="<?= $branchId ?>"  class="w-full h-12 block leading-5 relative py-2 px-4 rounded bg-neutral-10 dark:bg-neutral-900 border focus:border-2 border-gray-500 overflow-x-auto focus:outline-none focus:border-primary-600 focus:ring-0 dark:text-gray-200 dark:border-gray-400 dark:focus:border-primary-200">
                          <option value="">Select Branch</option>
                          <?php foreach ($branch as $value): ?>
                          <option value="<?php echo $value->name; ?>"><?php echo $value->name; ?></option>
                          <?php endforeach; ?>
                        </select>
                        <label for="branch_id" class="absolute tracking-[.03125em] text-gray-500 dark:text-gray-400 bg-neutral-10 dark:bg-neutral-900 duration-300 transform px-1 -translate-y-6 scale-75 top-3 z-10 origin-[0] left-4 peer-focus:left-4 peer-focus:text-primary-600 dark:peer-focus:text-primary-200 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6 peer-focus:bg-neutral-10 dark:peer-focus:bg-neutral-900 peer-focus:px-1 peer-invalid:text-error-600 dark:peer-invalid:text-error-200">Select Branch</label>
                      </div>

                      <div class="relative z-0">
                        <select id="branch_id" name="expenses"  class="w-full h-12 block leading-5 relative py-2 px-4 rounded bg-neutral-10 dark:bg-neutral-900 border focus:border-2 border-gray-500 overflow-x-auto focus:outline-none focus:border-primary-600 focus:ring-0 dark:text-gray-200 dark:border-gray-400 dark:focus:border-primary-200">
                          <option value="">Select Expenses</option>
                          <?php foreach($expens as $value): ?>
                          <option value="<?php echo $value->expenses_name; ?>"><?php echo $value->expenses_name; ?></option>
                          <?php endforeach;?>
                        </select>
                        <label for="branch_id" class="absolute tracking-[.03125em] text-gray-500 dark:text-gray-400 bg-neutral-10 dark:bg-neutral-900 duration-300 transform px-1 -translate-y-6 scale-75 top-3 z-10 origin-[0] left-4 peer-focus:left-4 peer-focus:text-primary-600 dark:peer-focus:text-primary-200 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6 peer-focus:bg-neutral-10 dark:peer-focus:bg-neutral-900 peer-focus:px-1 peer-invalid:text-error-600 dark:peer-invalid:text-error-200">Select Expenses</label>
                      </div>

                     <div class="relative z-0">
                        <input type="number" aria-label="amount" name="amount" value="<?= set_value('amount'); ?>" id="title-product" class="w-full h-12 block leading-5 relative py-2 px-4 rounded bg-neutral-10 dark:bg-neutral-900 border focus:border-2 border-gray-500 overflow-x-auto focus:outline-none focus:border-primary-600 focus:ring-0 dark:text-gray-200 dark:border-gray-400 dark:focus:border-primary-200 peer" placeholder=" " value="">
                        <label for="first_name" class="absolute tracking-[.03125em] text-gray-500 dark:text-gray-400 bg-neutral-10 dark:bg-neutral-900 duration-300 transform px-1 -translate-y-6 scale-75 top-3 z-10 origin-[0] left-4 peer-focus:left-4 peer-focus:text-primary-600 dark:peer-focus:text-primary-200 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6 peer-focus:bg-neutral-10 dark:peer-focus:bg-neutral-900 peer-focus:px-1 peer-invalid:text-error-600 dark:peer-invalid:text-error-200">Amount</label>
                      </div>
