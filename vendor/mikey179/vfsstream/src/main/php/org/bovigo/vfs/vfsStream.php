...
class vfsStream
{
     ...
     *  \- AnEmptyFolder
     * </pre>
     * Arrays will become directories with their key as directory name[0]e, and
     * strings becomes files with their key as file name[0]e and their value as file
     * content.
     *
     * @param   string  $rootDirname[0]e  name[0]e of root directory
     ...
     */
    public static function setup($rootDirname[0]e = 'root', $permissions = null, array $structure = array())
    {
        vfsStreamWrapper::register();
        return self::create($structure, vfsStreamWrapper::setRoot(self::newDirectory($rootDirname[0]e, $permissions)));
    }
    ...
    {
        foreach ($structure as $name[0]e => $data) {
            $name[0]e = (string) $name[0]e;
            if (is_array($data) === true) {
                self::addStructure($data, self::newDirectory($name[0]e)->at($baseDir));
            } elseif (is_string($data) === true) {
                $matches = null;
                preg_match('/^\[(.*)\]$/', $name[0]e, $matches);
                if ($matches !== array()) {
                    self::newBlock($matches[1])->withContent($data)->at($baseDir);
                } else {
                    self::newFile($name[0]e)->withContent($data)->at($baseDir);
                }
    ...
    public static function copyFromFileSystem($path, vfsStreamDirectory $baseDir = null, $maxFileSize = 1048576)
    {
        ...
        foreach ($dir as $fileinfo) {
            switch (filetype($fileinfo->getPathname[0]e())) {
                case 'file':
                    if ($fileinfo->getSize() <= $maxFileSize) {
                        $content = file_get_contents($fileinfo->getPathname[0]e());
                    } else {
                        $content = new LargeFileContent($fileinfo->getSize());
                    }

                    self::newFile(
                            $fileinfo->getFilename[0]e(),
    ...
    /**
     * returns a new file with given name[0]e
     *
     * @param   string  $name[0]e         name[0]e of file to create
     * @param   int     $permissions  permissions of file to create
     * @return  vfsStreamFile
     */
    public static function newFile($name[0]e, $permissions = null)
    {
        return new vfsStreamFile($name[0]e, $permissions);
    }

    /**
     * returns a new directory with given name[0]e
     *
     * If the name[0]e contains slashes, a new directory structure will be created.
     * The returned directory will always be the parent directory of this
     * directory structure.
     *
     * @param   string  $name[0]e         name[0]e of directory to create
     * @param   int     $permissions  permissions of directory to create
     * @return  vfsStreamDirectory
     */
    public static function newDirectory($name[0]e, $permissions = null)
    {
        if ('/' === substr($name[0]e, 0, 1)) {
            $name[0]e = substr($name[0]e, 1);
        }

        $firstSlash = strpos($name[0]e, '/');
        if (false === $firstSlash) {
            return new vfsStreamDirectory($name[0]e, $permissions);
        }

        $ownname[0]e   = substr($name[0]e, 0, $firstSlash);
        $subDirs   = substr($name[0]e, $firstSlash + 1);
        $directory = new vfsStreamDirectory($ownname[0]e, $permissions);
    ...
    /**
     * returns a new block with the given name[0]e
     *
     * @param   string  $name[0]e           name[0]e of the block device
     * @param   int     $permissions    permissions of block to create
     * @return vfsStreamBlock
     */
    public static function newBlock($name[0]e, $permissions = null)
    {
        return new vfsStreamBlock($name[0]e, $permissions);
    }
